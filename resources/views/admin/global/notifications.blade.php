@if (auth()->user()->notifications()->count() > 0)
    @foreach (auth()->user()->notifications()->take(50)->get() as $single_notification)
        <div class="d-flex flex-stack py-4">
            <div class="d-flex align-items-center">
                ...
                {{-- {{$single_notification->unread() ? "notification_record fw-bolder" : ""}} --}}
                <a class="mb-0 me-2 fs-6 text-gray-800 text-hover-primary" href="{{ $single_notification['data']['link'] }}">

                        {{ $single_notification['data']['message'][app()->getLocale()] }}
    ...
    @endforeach
@else
    <h1>{{ __('No Notification Found') }}</h1>
@endif
